    ...
    editorProps: {
      decorations(state) {
        ...
        state.doc.forEach((node, position, index) => {
          if (index === sourceHighlight.start) marks.push(Decoration.widget(position, () => {
            const label = document.createElement('div');
            label.className = 'generation-source';
            label.contentEditable = 'false';
            label.textContent = `Generated by ${sourceHighlight.author}`;
            return label;
          }, { side: -1 }));
          if (index >= sourceHighlight.start) marks.push(Decoration.node(position, position + node.nodeSize, {
            class: 'source-highlight',
            title: `Generated by ${sourceHighlight.author}`
          }));
        });
